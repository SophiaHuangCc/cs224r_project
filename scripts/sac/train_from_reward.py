...
Examples:
    ...
    python scripts/sac/train_from_reward.py \
        ...
        --reward generated_rewards/FetchPickAndPlace-v4_vanilla.py \
        --timesteps 200000

    # Eureka best reward
    python scripts/sac/train_from_reward.py \
        --env FetchSlide-v4 \
        --reward generated_rewards/eureka/FetchSlide-v4_best.py \
        --timesteps 1000000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train SAC+HER from a saved reward file.")
    ...
    parser.add_argument("--save-path", default=None,
                        help="Where to save the trained SAC model (default: derived from reward path)")
    ...
    if args.save_path is None:
        ...
        args.save_path = f"models/sac_her_from_reward/{reward_stem}"
    if args.metrics_path is None:
        args.metrics_path = f"{args.save_path}_metrics.json"

    print(f"Training SAC+HER on {args.env} with reward from {args.reward}")
