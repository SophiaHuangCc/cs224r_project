"""
Eureka-style Iterative Reward Design using SAC+HER.

Loop: LLM proposes reward → short SAC+HER training (delegated to `sac.training`)
...
This file only handles **reward generation / refinement**. SAC+HER training lives
in `scripts/sac/`, safety-metric tracking + evaluation lives in
`scripts/safety/`. The output of running this script is a set of generated
reward files under `generated_rewards/eureka_sac/`. A separate training script
...
DESIGN RULES:
1. ALWAYS have explicit return statements on ALL code paths (no implicit None returns)
2. Base reward primarily on achieved_goal vs desired_goal (task progress)
3. Do NOT rely on obs or action fields during HER relabeling (they may be None)
4. Return a scalar float, never None
...
def _ensure_return_statement(code: str) -> str:
    ...
    If the code ends with an expression (not return), wrap it in a return.
    Heuristic: add 'return' before the last expression if needed.
    """
    code = code.strip()
    lines = [line.rstrip() for line in code.split("\n") if line.strip()]
    
    # Check if last line is a return statement
    if lines and not lines[-1].lstrip().startswith("return"):
        # Last line is likely an expression — prefix with 'return '
        last_line = lines[-1]
        indent = len(last_line) - len(last_line.lstrip())
        lines[-1] = " " * indent + "return " + last_line.lstrip()
...
def eureka_loop_sac(
    ...
    """
    Eureka iterative reward refinement using SAC+HER.

    Reward generation lives here; SAC+HER training+evaluation is delegated to
    `sac.train_sac_her_with_reward` (which uses `safety.SafetyMetricWrapper` and
    `safety.evaluate_with_safety` under the hood). Each iteration's reward
    code and metrics are saved to `generated_rewards/eureka_sac/`. A separate
    ...
    history = []
    out_dir = "generated_rewards/eureka_sac"
    os.makedirs(out_dir, exist_ok=True)

    for iteration in range(n_iterations):
        print(f"\n{'='*60}")
        print(f"EUREKA (SAC+HER) ITERATION {iteration + 1}/{n_iterations}")
        print(f"{'='*60}")
    ...
    # Save best reward as `<env_id>_best.py` for downstream training
...
if __name__ == "__main__":
    ...
    print(f"Running Eureka (SAC+HER) loop for {env_id}...")
    ...
    # Summary
    print("\n\n" + "=" * 60)
    print("EUREKA SUMMARY (SAC+HER — no safety feedback to LLM)")
    print("=" * 60)
    for h in history:
        m = h["metrics"]
        print(
